...
- Link rebind preserves interpolated URLs inside JavaScript template literals (including inline scripts): `${imageMap[item.id]}` is executable per-item selection, not a missing filename to replace with one artifact. This static mutation guard does not prove dynamic execution; literal placeholders and missing static dependencies still require repair.
...
- A no-ready-instance failure during late-fill preparation is retryable `INSTANCE_UNAVAILABLE`, not a branch-contract defect. Confirmed live process and listening port override advisory cooldown/readiness labels; such a candidate routes immediately. Persisted `availability_wait` branches remain compatible: the existing continuation worker rechecks routing without consuming repair attempts, holding a device lock, or excluding a provider that never executed. Cancellation remains checked in at most one-second sleep slices. Hard-unavailable truth and explicit execution controls remain authoritative.
- Final local-dependency checks can discover a target that never had an original branch. Such a check retains its exact target and consumer-file context and enters the existing bounded terminal repair projection with a stable target identity. Pending, active, failed, cancelled, and historical target work prevents fresh identities from resetting retry limits. Missing-link diagnostics alone do not prove fulfillment; the saved dependency must exist and pass final validation.
- Composed-site image repairs retain the same target-bound authority during Late Fill handoff. Repair identity comes from the target and repair provenance, independently of missing or changed retry-action labels. Existing target bytes are repair input, never fulfillment evidence; only a branch-local write to that target may close the repair. Earlier writes from other branches cannot satisfy it. Final composition validation remains required.
...
- composed multi-page sites treat HTML, shared CSS, and dynamic JavaScript as one read-only repair cohort while retaining one exact writable target per repair branch; a detached image dump is diagnostic evidence, never successful composition
...
- when dependency closure replaces a referenced older/retained file with one current authoritative artifact, the bundle keeps the exact original local reference as a rewrite-only alias to that selected file; it never copies both generations, and ambiguous authority remains a failed link check
