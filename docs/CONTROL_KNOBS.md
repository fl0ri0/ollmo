...
- `OLLMO_LATE_FILL_AVAILABILITY_POLL_SEC`: route recheck interval for persisted availability waits; default `5` seconds, clamped to `1`–`60`. Waiting stays on the same response/branch without consuming repair attempts or holding a backend lock. Cancellation is checked in at most one-second sleep slices. Confirmed live process plus listening port now override cooldown labels and route immediately; cooldown alone does not put such models into waiting. Explicit execution controls remain authoritative. No model is started by this mechanism.
...
Qwen3 Base splits nonempty newline-delimited text inside MLX-Audio and applies `max_tokens` to each segment, so `qwen3_tts_sentence_chunks_v1` compares the 56-second boundary with the longest Base segment rather than aggregate narration length. A multiline Base request can therefore produce many minutes in one backend call. Its adaptive budget is derived from the longest segment and explicitly records that the limit applies per backend segment; at 56 seconds the formula requests 1150 tokens, leaving 50 tokens (four codec seconds) below the per-segment 1200-token maximum. A single Base line above that boundary enters Ollmo's verified chunk path. CustomVoice and VoiceDesign use a 28-second full-source trigger so moderate designed-voice passages stay in one stochastic voice realization. Unknown/default planner callers retain the conservative 16-second fallback. Runtime still targets chunks of at most 10 estimated seconds once the applicable trigger is crossed, preserves ordered source spans, applies the adaptive budget separately to each backend call, verifies every returned chunk, and joins only compatible uncompressed PCM WAV frames into one persisted artifact. A single-sequence Qwen3 WAV that lands exactly on its declared codec-token ceiling is blocked as generation-limit exhaustion even if residual energy would otherwise resemble active signal. Multiline Base is marked `segmented_sequence`; Runtime does not claim an exact single-sequence cap proof for that aggregate. Chunk verification additionally rejects internal inactive gaps longer than 3 seconds. These checks add negative evidence; they do not replace or weaken the existing no-signal, effective-duration, trailing-silence, source-digest, or Closure gates.
...
Required-image retries have a separate user-only exhausted-pool policy. The endpoint may attach `explicit_image_excluded_pool_retry_v1` only to an explicitly retried image branch whose prior error is `NO_COMPATIBLE_INSTANCE`; exclusions remain intact and `automatic_follow_up_allowed=false` prevents the result from entering the automatic repair loop. Runtime first requires a complete branch-local image prompt contract and then refreshes current live candidate truth. A ready non-excluded image instance always wins. Only when every currently usable compatible image instance is already excluded may one be re-admitted for that click, recorded as `selection_policy=excluded_reuse_for_explicit_image_retry`, `excluded_reuse_policy_id=explicit_image_excluded_pool_retry_v1`, and `excluded_instance_reuse_reason=user_triggered_retry_after_ready_image_pool_exhausted`. Missing prompts, dependency/contract repair, external-input blocks, stale-only liveness, other error codes, other capabilities, and automatic follow-ups fail closed.
...
Monitor and reference-run evidence is frame-scoped even when recovery appends a successor under the same response id. The monitor deduplicates by `(response_id, frame_id)`, using frame sequence only when the id is unavailable, so a failed frame and its later successful successor each receive one append-only report. Reference export first resolves the authoritative latest indexed frame and accepts only a monitor report matching that exact frame; stale-only or frame-less evidence fails closed rather than being attributed by response id. If the same immutable frame has multiple observer records, the last appended matching record is the publication authority. Neither monitoring nor export rewrites response frames or historical reports.
...
For a composed website with multiple HTML files and generated images, path presence alone is not completion evidence. Runtime rejects a detached `ollmo-generated-media` catch-all section, preserves ordered image prompt/path evidence, and promotes one bounded in-place repair per existing HTML, CSS, or JavaScript target. Each branch receives its target bytes plus read-only cohort snapshots; it may replace only that exact path. Closure completes only after the saved cohort binds the generated images to its site roles and exposes deliberate image layout CSS.
...
- If no → inspect the **Runtime/Substrate knob** first. Ghost may propose a graph patch in response to repair feedback; Runtime validates, applies and records only authorized patches and owns closure truth. Then inspect the **Resolver knob** if continuation selection or dependency readiness was wrong.
...
`infer_timeout_sec` is forwarded to Ollama text chat, including attached-text and preparation-phase requests, as it is to other chat backends. The inference endpoint defaults to 1200 seconds and retains its existing bounds; explicit request values take precedence. The low-level chat helper retains a 180-second default for callers that supply no timeout. This is a request timeout, not a progress-aware generation watchdog. Ordinary chat timeout errors do not suggest PDF controls; PDF requests retain PDF-specific advice.

### Causal transition observation budget

`ollmo_services/events.py` names `CAUSAL_TRANSITION_RESERVE` (256 compact
records per causal scope), separate from the ordinary 1,024 records and the
32-record finalizer overflow reserve. This is an observation bound, not a work,
wait, retry or scheduling limit. Paired spans reserve both endpoints; exhaustion
increments explicit transition-drop counters and leaves execution unchanged.
There is no new environment override or production configuration change.
See [CAUSAL_TELEMETRY](CAUSAL_TELEMETRY.md#bounded-branch-handoff-witnesses).

### Snapshot preparation memory budgets

`_SNAPSHOT_SERIALIZATION_REUSE_MAX_BYTES` in
`ollmo_services/response_frames.py` allows 8 MiB of retained encoded input,
output and normalization metadata per compaction (container overhead is
additional). It bounds optional child-snapshot preparation reuse, never durable
state. Exhaustion uses the original normalization/serialization path. Root
writes, CAS hashing, fresh sidecar verification and all authority checks remain
unchanged; no process-wide cache or environment switch is introduced.

`_SNAPSHOT_SIZE_PREPARATION_MAX_BYTES` independently permits 8 MiB of retained
typed input/size representations in the same compaction owner. It reuses only
pure JSON byte-size computation. Path/depth rules, split eligibility, sibling
reservations, ref budgets, media handling and CAS checks remain fresh. Unknown
input types, changed owner/policy/storage bindings or exhausted memory take the
full calculation path. The two budgets are separate and exclude container
overhead; neither truncates durable content.
